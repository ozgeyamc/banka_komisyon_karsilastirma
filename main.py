...
def _extract_rows(table, kategori) -> List[UcretSatiri]:
    ...
    def fc(keys):
        """Geliştirilmiş header bulma - tam match sonra partial match"""
        # Tam match
    ...
    if cm == -1 or ca1 == -1:
        ...
        print(f"[DEBUG] Kolon indeksleri - Masraf:{cm} AsgariT:{ca1} AsgariO:{ca2} AzamiT:{cz1} AzamiO:{cz2}", file=sys.stderr)
